...
| UI (PySide6) | A′, A′-plus, B′, B′ companion, B24, B9a, B27, C′, B34, B41, B44, B22, B10, Curve Inspector, and Manage Reference Data complete | 541 |
| Exports (calendar / ICS, XLSX) | Complete | 27 |
| Tools (admin scripts) | Complete | 71 |
| Build info | Complete | 3 |

1462 tests pass in ~16 seconds. If any test fails on a fresh checkout, treat that as the first bug to fix.
...
The module imports from `domain`, `persistence`, `engine.projection`, `engine.fgc`, `engine.conglomerate_report`, `exports.calendar`, `importers.detection` (which dispatches to the appropriate broker loader — XP, BTG, or BB), `importers.loader_types` (for `LoadResult`), `importers.xp_mapper` (for `parse_brazilian_money`), and `ui.curve_inspector`. It introduces no new architectural layer between itself and those — direct calls, no service or presenter layer.
...
The **Manage Reference Data** dialog (`ui/manage_reference_data.py`, `ManageReferenceDataDialog`) is a modal opened from View ▸ Manage Reference Data. It has three tabs: **Issuers** (delete orphan issuers, FK-guarded), **Conglomerates** (rename/merge/dissolve, backed by `IssuerRepository.rename_conglomerate` / `dissolve_conglomerate` with curation-memory sync), and **Custodians** (rename/clear across investments, backed by `InvestmentRepository.rename_custodian` / `clear_custodian`). On close, `MainWindow.refresh_table()` re-reads investments from the DB; a projection-cache re-pair step (`dataclasses.replace` re-points each cached `ProjectionResult` to the freshly-read investment of the same ID) ensures conglomerate label changes propagate without a manual re-projection. (Shipped 2026-06-17, commits `86212f6`–`9e0d55c`.)
...
**1462 tests, ~16 second runtime, no skips.** The test suite is the spec; if behavior changes, the test changes first.
